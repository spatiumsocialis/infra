...
// AddToCircle adds the current user to the specified circle
func (s common.Service) AddToCircle(w http.ResponseWriter, r *http.Request) {
...
func RemoveFromCircle(w http.ResponseWriter, r *http.Request) {
	// Get the current user
	user, err := common.GetUser(r)
	if err != nil {
	...
	// Get user to remove
	var userToRemove common.User
	if result := s.DB.Find(&userToRemove, common.User{ID: removeUserID}); result.Error != nil {
